...
function travelRec_toPO_scheduled(type) {
	...
	var domainURL = 'https://4511400.app.netsuite.com';
	...
	if (recs.length > 0) {
		...
		//recsGrouped.forEach(function(group) {
			...
			if(currentGroup.length != 1) {
				...
				var newPOMulti = nlapiCreateRecord('purchaseorder', { recordmode: 'dynamic' });
				
				newPOMulti.setFieldValue('customform', 105)
				...
				newPOMulti.setCurrentLineItemValue('item', 'custcol_jfrog_travel_po_link_col', domainURL + nlapiResolveURL('RECORD', 'customrecord_jfrog_travel_po', currentGroup[x].internalid));
	            ...
	            ogTravelRecsToUpdate.push(currentGroup[x].travelNumber)
						...
						nlapiLogExecution('debug', 'error in the currentGroup loop - multiplePO', err)
					}
	            ...
	            var POCreated = nlapiSubmitRecord(newPOMulti, null, true);
				
	         	//nlapiSubmitField('purchaseorder', POCreated, 'employee', currentGroup[0].budgetOwner);
				//nlapiSubmitField('purchaseorder', POCreated, 'custbody5', currentGroup[0].budgetOwner);
			...
			if(currentGroup.length === 1){
				...
				for( var i = 0; i<currentGroup.length; i++) {
					...
					newPO.setFieldValue('customform', 105)
					...
					newPO.setFieldValue('employee',parseInt(currentGroup[i].budgetOwner));
					newPO.setFieldValue('custbody5',parseInt(currentGroup[i].budgetOwner)); //Buyer
					...
					newPO.setCurrentLineItemValue('item', 'amount', currentGroup[i].amount);
					...
					newPO.setCurrentLineItemValue('item', 'custcol_jfrog_travel_po_link_col', domainURL + nlapiResolveURL('RECORD', 'customrecord_jfrog_travel_po', currentGroup[i].internalid));
			
		            newPO.commitLineItem('item');
		   


		                var SinglePOCreated = nlapiSubmitRecord(newPO, null, true);
			            nlapiLogExecution('debug', 'POCreated', SinglePOCreated)
			            
			         	//nlapiSubmitField('purchaseorder', SinglePOCreated, 'employee', currentGroup[i].budgetOwner);
						//nlapiSubmitField('purchaseorder', SinglePOCreated, 'custbody5', currentGroup[i].budgetOwner);
