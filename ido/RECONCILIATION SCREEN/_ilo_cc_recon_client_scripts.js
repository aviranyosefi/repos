...
var checksReconciled = getAllReconChecks();
...
function recon_client_PageInit(type){
	...
	if(booktypeSelected == 'b') {
		nlapiSetFieldValue('custpage_summary_booktype', 'Local Book');
	}
...
function saveRecon() {
	...
	var stringToUpdateLength = JSON.stringify(toUpdate.length);
	...
	recoveryRec.setFieldValue('custrecord_ilo_recon_return', '');
	recoveryRec.setFieldValue('custrecord_ilo_recovery_point', 0);
	recoveryRec.setFieldValue('custrecord_ilo_recon_curr_session', stringToUpdate);
	recoveryRec.setFieldValue('custrecord_ilo_recon_length', stringToUpdateLength);
	recoveryRec.setFieldValue('custrecord_ilo_recon_memo_return', '');
...
function unreconcile() {
	...
	for (var j = 0; j<unReconLines; j++) {
		...
		var getCheckTranNum = nlapiGetLineItemValue('custpage_loaded_sublist', 'custpage_loadedsublist_tranid', j +1);
...
for(var x = 0; x<checksReconciled.length; x++) {
	
	if(toUnReconcile[i].tranNum == checksReconciled[x].check_tranNum) {
		
		nlapiDeleteRecord('customrecord_ilo_recon_bank_check', checksReconciled[x].check_internalid);
			...
			if (allreconChecks != null) {
				allreconChecks.forEach(function(line) {
					...
					reconChecks.push({
						...
						check_internalid : line.getValue('internalid')
