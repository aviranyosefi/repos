...
function exchangeRateUpdate() {
	...
	var USDline = '';
		...
		var toUpdate = [];
		//toUpdate array are the currencies that are both exsisting in the sy
		...
		for(var m = 0; m<toUpdate.length; m++) {
			if(toUpdate[m] == 'ILS') {
				USDline += usd+","+toUpdate[m]+","+ROTbase(1/exRateArr.USD,exRateArr[toUpdate[m]]).toFixed(5)+","+effectiveDate+ "\n";
				}
