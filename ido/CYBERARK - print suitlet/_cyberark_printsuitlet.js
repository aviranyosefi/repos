	 ...
	 if ( request.getMethod() == 'GET' )  {
		    ...
		    htmlHeader.setDefaultValue("<p style='font-size:20px'>Please fill in the form below in order to send this invoice to the customer. Then click 'Send'.<br>The invoice will be added as an attachment.</p><br><br>");
			 ...
			 var recipientGroup = emailForm.addFieldGroup( 'custpage_recipient_group', 'Recipients');
				...
				var psPage = createPSDetailPage(invoiceID);
			...
			emailSubject.setDefaultValue('CyberArk Invoice '+invoiceName)
...
//			
			...
			/////////////////////HIDDEN FIELDS//////////////////////////////
			 emailForm.setScript('customscript_dev_ps_email_control_cs')
			 		 emailForm.addButton('custpage_preview_invoice','Preview Invoice', 'previewInvoice();');
...
//				}
				...
				var psPage = createPSDetailPage(invoiceRecID);
			    var psDetailsPDF = downloadDataPDF(psPage, response, true); //print pdf
			    ...
			    arrayWithUrls.push(currInvoiceURL)
			    arrayWithUrls.push(psDetailsURL)
			    var attachement =  mergePDFandServe(arrayWithUrls, _invoice_number, true)
...
function createPSDetailPage(invoiceID) {
	 ...
	 line+= '<td colspan="3" class="left">' + arr[x].engineer_text + '</td>';
	 line+= '<td colspan="3" class="left">' +  arr[x].startDate + '</td>';
	 line+= '<td colspan="3" class="left">' +  arr[x].endDate + '</td>';
	 line+= '<td colspan="3" class="center">' +  arr[x].duration + '</td>';
	 line+= '<td colspan="3" class="left">' +  arr[x].psDay + '</td>';
	 line+= '<td colspan="3" class="left">' +  arr[x].location_type + '</td>';
	 line+= '<td colspan="3" class="left">' +  arr[x].service_type + '</td></tr>';
...
function mergePDFandServe(arrayWithUrls, _invoice_number, asAttachment) {
	...
	xml += "<pdfset>"
		...
		arrayWithUrls.map(function (x) {
			...
			xml += '<pdf src="https://4678143-sb4.app.netsuite.com' + cleanPdfURL + '"></pdf>'
		})
	xml += "</pdfset>"
	...
	response.setContentType('PDF', 'Invoice - '+_invoice_number+'.pdf', 'inline');
	response.write(file);
	}
	...
	file.setName('Invoice - '+_invoice_number+'.pdf');
...
function downloadDataPDF(data, response, asAttachment) {
