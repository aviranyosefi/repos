...
//		};
		...
		resultsSubList.setLineItemValue('custpage_resultssublist_docid', j +1, invoiceArr[j]);
	     ...
	     //Add to the Suitelet a check box script parameter called Check Box Required
	       var field = printChoiceForm.addField('custscript_ilo_tomailresults', 'longtext', 'tomailresults');
	       field.setDefaultValue('hello');	       
		...
		if(request.getParameter('custpage_ilo_checkifprintormail') == 'mail') {
			...
			for (var k = 0; k<emailArr.length; k++) {
				  ...
				  toSend.push ({
				            ...
				            printType : emailArr[k].printType,
				            documentName: emailArr[k].docName,
				            customerName: emailArr[k].customerName,
				  });
			...
			for (var i = 0; i < toSend.length; i++) {
				  ...
				  var somedocName = toSend[i].documentName;
				  var somecust = toSend[i].customerID;
				  var somecustName = toSend[i].customerName;
				  var somecustNameNoDigits = somecustName.replace(/[0-9]/g, '');
				  var someprintType = toSend[i].printType;
				  
				  if(someprintType == 'a') {
					  nlapiSubmitField('invoice', somedocIDs,'custbodycustbody_ilo_print_draft', 'T');
					 
				  }
				  if(someprintType == 'c') {
					  nlapiSubmitField('invoice', somedocIDs,'custbodycustbody_ilo_print_draft', 'F');
				  }
						...
						nlapiSendEmail(fromId, somemails, sbj, msg, 'billing@optimove.com', null, attachRec, attachment);
						  ...
						  if(someprintType == 'c'){
//							  nlapiSubmitField('invoice', somedocIDs,'custbodycustbody_ilo_print_draft', 'F');
							  nlapiSubmitField('invoice', somedocIDs, 'custbody_ilo_org_printed', 'T');
							  
						  }
			
				}
			 var toSendcheckmail=[];
			 var checkOriginalmail = request.getParameter('custpage_tomailresults');
				var checkOGprintmail = JSON.parse(checkOriginalmail);
				for (var k = 0; k<checkOGprintmail.length; k++) {
					  
					toSendcheckmail.push ({
						  		documentID : checkOGprintmail[k].docID,
					            mail: checkOGprintmail[k].contMail,
					            customerID : checkOGprintmail[k].customerID,
					            printType : checkOGprintmail[k].printType
					  });

					}
			
			for (var i = 0; i < toSendcheckmail.length; i++) {

				  var p_somedocIDsmail = toSendcheckmail[i].documentID;
				  var p_someprintTypemail = toSendcheckmail[i].printType;
				  
//				  if(p_someprintTypemail == 'a') {
//					  nlapiSubmitField('invoice', somedocIDsmail,'custbodycustbody_ilo_print_draft', 'T');
//					 
//				  }
//				  if(p_someprintTypemail == 'c'){
//					  nlapiSubmitField('invoice', p_somedocIDsmail,'custbodycustbody_ilo_print_draft', 'F');
//					  nlapiSubmitField('invoice', p_somedocIDsmail, 'custbody_ilo_org_printed', 'T');
//					  
//				  }
