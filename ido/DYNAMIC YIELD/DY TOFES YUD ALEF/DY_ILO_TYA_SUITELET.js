...
//		nlapiLogExecution('debug', 'all', JSON.stringify(all, null ,2))
		...
		for(var x = 0; x<typeKeys.length; x++) {
			...
			if(currType.length == 0) {
				...
				for(var i = 0; i<currType.length; i++) {
					 ...
					 costSale_sum.push(parseFloat(currType[i].cost_sale))
					 ...
					 dprnSale_sum.push(parseFloat(currType[i].dprn_sale));
				...
				totalObj.cost_sale = costSale_sum.reduce(add, 0).toFixed(2);
				...
				totalObj.dprn_sale = dprnSale_sum.reduce(add, 0).toFixed(2);
		...
		for (var i=0; i<allArr.length; ++i) {
		    ...
		    for (var j=0; j<current.length; j++) {
		    	...
		    	if(current[j].asset_name == 'TOTAL') {
		    		...
		    		grand_costSale_sum.push(parseFloat(current[j].cost_sale));
		    		...
		    		grand_dprnSale_sum.push(parseFloat(current[j].dprn_sale));
		...
		GrandTotalObj.cost_sale = grand_costSale_sum.reduce(add, 0).toFixed(2);
		...
		GrandTotalObj.dprn_sale = grand_dprnSale_sum.reduce(add, 0).toFixed(2);
	...
	var htmlLines = HTMLlines.toString().replace(/,/g,'');
		...
		var tableGroup = resultsForm.addFieldGroup('custpage_table_group', 'Results');
		     ...
		     htmlHeader.setDefaultValue("<span style='font-size:18px'>"+subsidName+" - "+accBookName+"</span><br><br>" +
		     		...
		     		"<span style='font-size:18px'>"+subsidTikNik+" : תיק ניכויים</span><br>" +
		     		"<span style='font-size:18px'>"+subsidVatReg+" : .ח.פ</span>");
		     ...
		     "<th class='tya_header' style='width:150px'>COST - SALE</th>"+
		     ...
		     "<th class='tya_header' style='width:150px'>DPRN - SALE</th>"+
...
function getAllActivities(accBook, subsidiary, search) {
	...
	if (allSelection != null) {
		...
		theResults.push({
			...
			internalID : line.getValue(cols[19]),
			assetID : line.getValue(cols[16])
		});
...
function makeSingleLine(assetName, AllLines, OBAllLines)  {
	...
	var OB_single = OBAllLines;
		...
		for(var i = 0; i<inRange_single.length; i++) {
			...
			if(indicator == 'COST' && trantype == 'Sale') {
				cost_sumSale.push(parseFloat(inRange_single[i].net_amount));
			}
			...
			if(indicator == 'DPRN' && trantype == 'Sale') {
				dprn_sumSale.push(parseFloat(inRange_single[i].net_amount))	
			}
	...
	int_cost_sale = cost_sumSale.reduce(add, 0)
	...
	lineObj.cost_sale = int_cost_sale.toFixed(2);
	...
	int_dprn_sale = dprn_sumSale.reduce(add, 0);
	...
	lineObj.dprn_sale = int_dprn_sale.toFixed(2);
	...
	for(var i = 0; i<OB_single.length; i++) {
		...
		if(indicator == 'COST' && (trantype == 'Acquisition' || trantype == 'Sale' || trantype == 'Disposal' || trantype == 'Write-down' )) {
			costOB.push(parseFloat(OB_single[i].net_amount));	
		}

		
		if(indicator == 'DPRN' && (trantype == 'Depreciation' || trantype == 'Sale' || trantype == 'Disposal' || trantype == 'Write-down')) {
	...
	var totalCost = int_cost_acq+int_cost_sale+int_cost_disposal+int_cost_writedown+int_cost_ob
	var totalDprn = int_dprn_dprn+int_dprn_sale+int_dprn_disposal+int_dprn_writedown+int_dprn_ob
...
//				 htmlLine = "<tr onmouseover='this.classname=" + mouseoverclass + "'>" +
				htmlLine = "<tr>" +
				    ...
				    "<td class='tya_cell' data-attribute='cost_acq' style='width:150px'>"+element.cost_sale+"</td>"+
				    ...
				    "<td class='tya_cell' data-attribute='dprn_dprn' style='width:150px'>"+element.dprn_sale+"</td>"+
			...
			if(element.asset_name == 'TOTAL' || element.asset_name == 'GRAND TOTAL') {
				 htmlLine = "<tr>"+
				    ...
				    "<td class='tya_cell_total' data-attribute='cost_acq' style='width:150px; font-weight: bold'>"+element.cost_sale+"</td>"+
				    ...
				    "<td class='tya_cell_total' data-attribute='dprn_dprn' style='width:150px; font-weight: bold'>"+element.dprn_sale+"</td>"+
