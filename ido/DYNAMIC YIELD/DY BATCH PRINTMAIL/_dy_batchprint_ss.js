...
function scheduled(type) {
	...
	var arr = ctx.getSetting('SCRIPT', 'custscript_dy_jobstorun')
	var printPref = ctx.getSetting('SCRIPT', 'custscript_dy_printpreference')
	var emailSender = ctx.getSetting('SCRIPT', 'custscript_dy_emailsender')
	var emailReciever = ctx.getSetting('SCRIPT', 'custscript_dy_emailreciver')
	var printOrMail = ctx.getSetting('SCRIPT', 'custscript_dy_printormail')
	...
	if(printOrMail == 'Mail') {
		...
		for(var x = 0; x<getSelection.length; x++) {
			...
			try{
				...
				var sbj = 'You have a new Invoice from Dynamic Yield - ' + getSelection[x].inv_name;
...
function invSearch(arr) {
	...
	var res = [];
	  ...
	  columnsInvoice[1] = new nlobjSearchColumn('custentity_dy_batchprintmail_email', 'customer');
	  ...
	  if (s != null) {
	    ...
	    for (var i = 0; i < s.length; i++) {
	        ...
	        customerMail: s[i].getValue('custentity_dy_batchprintmail_email', 'customer'),
