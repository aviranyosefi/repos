...
function opt_sendEmails(type) {
	...
	function invSearch(invoiceArr) {
	  ...
	  var columnsInvoice = new Array();
		...
		columnsInvoice[12] = new nlobjSearchColumn('custbody_sbtech_email_comment')
	  ...
	  if (s != null) {
	    ...
	    for (var i = 0; i < s.length; i++) {
	      ...
	      toSend.push({
	        ...
	        email_comment : s[i].getValue('custbody_sbtech_email_comment')
...
//	nlapiLogExecution("DEBUG", 'scriptParameter - id', data);
	...
	if(toSend[x].print_Type != 'd') { //external only - send to customer
		...
		nlapiSendEmail(fromId, toSend[x].mail, sbj, msg, cc_mail, null, attachRec, attachment);
	...
	if(toSend[x].print_Type == 'd') { //internal only
		...
		var from = fromId  //Authors' Internal ID 
		var attachment = nlapiPrintRecord('TRANSACTION',toSend[x].inv_id,'PDF',null);
		var sbj = company +" – invoice approval - " + toSend[x].customerName + " " + toSend[x].trandate +" "+ toSend[x].inv_name;
		...
		//multiple email addresses
		//Recipient is a comma separated list of email addresses
		nlapiSendEmail(fromId,toSend[x].salesrep_mail, sbj, msg, cc_mail, null, attachRec, attachment);
