...
function approve_tran(request, response) {
    ...
    if (request.getMethod() == 'GET') {
       ...
       // fieldtran.setDefaultValue(tranid);
        ...
        var cols_to_ignore = ['accounttype', 'internalid', 'linesequencenumber', 'symbol', 'item'];
        var htmlLines = "";
        ...
        for (var colind in columns) {
            if (columns.hasOwnProperty(colind) && cols_to_ignore.indexOf(columns[colind].name) == -1) {
                if(columns[colind].name =='memomain') {
                    htmlLines +=
                        "<th class='tya_header' style='width:150px; display:none;'>" + columns[colind].label+ "</th>";	
                }else{
                     htmlLines +=
       ...
       "<th class='tya_header' style='width:150px'>Status</th>";
        ...
        for (var rowindex in arrResSorted) {
            ...
            for (var colind in columns) {
                ...
                else if(columns[colind].name == "memomain") {
                	htmlLine += "<td class='tya_cell' style='display:none'>" + arrResSorted[rowindex].getValue(columns[colind]) + "</td>";
                }
                else {
        ...
        var exportCSVButton = form.addButton('custpage_exportcsv', 'Export CSV', null);
        form.addSubmitButton('Load Again');
        form.setScript('customscript_opt_export_csv_budget_owner')
        response.writePage(form);
    }
}
