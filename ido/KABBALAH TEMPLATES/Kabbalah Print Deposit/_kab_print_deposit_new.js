...
function todayDate() { 
	...
	function getAllRecs(depositID) {
		...
		var searchItems= nlapiLoadSearch(null, 'customsearch128');
		    ...
		    for (rs in resultslice) {
				...
				if (allItems != null) {
					allItems.forEach(function(line) {
						...
						amount : checkIfMinus(document, line.getValue(cols[6])),
						bankBranch : line.getValue(cols[8]),
						bankNum : line.getValue(cols[9]),
						checkDue : line.getValue(cols[10]),
						bankAcc : line.getValue(cols[11]),
						checkNum : line.getValue(cols[12])
