...
//		sum_last_assembly.setDisplayType('inline');
		...
		nlapiLogExecution('DEBUG', 'usage left - beginning of else', rt);
...
//				try{
//					var statusCheck = checkInStock(itemID, serialSelected);
//		}catch(err) {
//			nlapiLogExecution('debug', 'statusCheck', err)
//		}

		//nlapiLogExecution('debug', 'statusCheck', JSON.stringify(statusCheck))
//		var stockStatus = checkStockStatus(statusCheck, serialSelected);
//		sum_status.setDefaultValue(stockStatus);

	
		
		try{
			var serialID =  getSerialID(itemID, serialSelected);
		}catch(err) {
...
//		var setLocation = setHeaderLocation(itemID, serialSelected)
//		
//			var loc = getAllSerialsLastCheck(serialSelected)
//			var checkloc = JSON.stringify(loc)
//			nlapiLogExecution('debug', 'what is loc', JSON.stringify(loc))
//			
//			if(checkloc != '[]') {
//				
//				//nlapiLogExecution('debug', 'loc', JSON.stringify(loc[0].location))
//				var locationName = nlapiLookupField('location', loc[0].location, 'name')
//						sum_location.setDefaultValue(locationName);
//				
//			}
//		if(loc == '[]') {
//			
//	
//					sum_location.setDefaultValue(' ');
//			
//		}
...
//			var a = resultsSubList.addField('custpage_resultssublist_test'+JSON.stringify(i+1), 'url','test');
//			resultsSubList.setLineItemValue('custpage_resultssublist_test'+JSON.stringify(i+1), i + 1,results[i].trannumber_view);
//			res_viewTran.setLinkText(results[i].trannumber);
//			resultsSubList.setLinkText('custpage_resultssublist_viewemp', i + 1,results[i].trannumber);
...
//			if(results[0].trantype == 'Build') {
//				isBuild = results[0].tranid;
//				tranNum = results[0].trannumber;
//				
//			}
//			
//			if(results[0].trantype == 'Unbuild') {
//				isBuild = results[0].tranid;
//				tranNum = results[0].trannumber;
//				
//			}
...
function getTransactions(serial) {
	...
	function getSerialID(itemid, serialselected) {
