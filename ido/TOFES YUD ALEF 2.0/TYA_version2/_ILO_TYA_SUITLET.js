...
function getAllActivities(accBook, subsidiary, search) {
	...
	if (allSelection != null) {
		...
		theResults.push({
			...
			asset_name : line.getValue(cols[3]).replaceAll(',',''),
...
function getOpenBalances_Primary(searchid, selectedSubsid) {
	...
	if (allSelection != null) {
		...
		//	if(line.getText(cols[7]) == subsidName) {
                              ...
                              asset_name : line.getValue(cols[3]).replaceAll(',','').replaceAll('↵',''),
							  asset_type : line.getText(cols[4]),
				  			  asset_lifetime : line.getValue(cols[5]),
							  purchase_date : line.getValue(cols[6]),
							  subsidiary : line.getText(cols[7]),
	    ...
	    for (rs in resultslice) {
			...
			if (allassets != null) {
				allassets.forEach(function(line) {
					...
					resultsAssets.push(line.getValue('name').replaceAll(',','').replaceAll('↵',''));
...
//				 htmlLine = "<tr onmouseover='this.classname=" + mouseoverclass + "'>" +
				htmlLine = "<tr>" +
				    ...
				    "<td class='tya_cell' data-attribute='assetdescription' style='width:150px'>"+element.asset_description.replaceAll('\n','').replaceAll('#','')+"</td>"+
			...
			if(element.asset_name == 'TOTAL' || element.asset_name == 'GRAND TOTAL') {
				 htmlLine = "<tr>"+
				    ...
				    "<td class='tya_cell_total' data-attribute='assetdescription' style='width:150px; font-weight: bold'>"+element.asset_description.replaceAll('\n','').replaceAll('#','')+"</td>"+
