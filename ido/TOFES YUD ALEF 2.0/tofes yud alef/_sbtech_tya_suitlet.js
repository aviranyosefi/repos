...
//				for(var j = 0; j<selectedAssetTypes.length; j++) {
//					
//					if(allTypes[i].internalid == selectedAssetTypes[j]) {
//						
//						getAllTypes.push(allTypes[i])
//					}
//				}
//			}
//			
//		}
//		nlapiLogExecution('debug', 'getAllTypes', JSON.stringify(getAllTypes))
	...
	var htmlLines = HTMLlines.toString().replace(/,/g,'');
		...
		var tableGroup = resultsForm.addFieldGroup('custpage_table_group', 'Table');
		 ...
		 var htmlHeader = resultsForm.addField('custpage_header', 'inlinehtml');
		     ...
		     "<tr>"+
		     "<th class='tya_header' style='width:150px'></th>"+
...
function getAllActivities(accBook, subsidiary, search) {
	...
	if (allSelection != null) {
		...
		theResults.push({
			...
			internalID : line.getValue(cols[20]),
...
//				 htmlLine = "<tr onmouseover='this.classname=" + mouseoverclass + "'>" +
				htmlLine = "<tr>" +
				    "<td class='tya_cell' data-attribute='internalid' style='width:150px'><a href = "+assetView(element.internalid)+">View</a></td>"+
			...
			if(element.asset_name == 'TOTAL' || element.asset_name == 'GRAND TOTAL') {
				 htmlLine = "<tr>"+
				    "<td class='tya_cell_total' data-attribute='internalid' style='width:150px; font-weight: bold;'></td>"+
