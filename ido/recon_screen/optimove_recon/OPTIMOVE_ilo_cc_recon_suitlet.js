...
function cc_recon() {
	...
	if (request.getMethod() == 'GET') {
		...
		selectSubsidiary.setDefaultValue('2');
...
//		toDate.setDefaultValue('28/02/17');
		...
		sessionNumber.setDefaultValue('<font size="3"><b> Session ID : '+ newReconSesNumber + '-U</b>');
	...
	else if (request.getParameter('custpage_ilo_continuerecon') == 'cont') {
		
		var markedTrans = request.getParameter('custpage_ilo_marked_lines');


	      var param = new Array();

	      param['item'] = markedTrans;

	      //Redirect to second Suitelet Page

	      response.sendRedirect("SUITELET", "customscript_ilo_recon_cont_suitelet", "customdeploy_ilo_recon_cont_suitelet_dep", false, param);
		...
		if (selectedBook == 'a' || 'b') {
			...
			try {
				...
				var resultsSubList = reconForm
						.addSubList('custpage_results_sublist', 'editor',
								'Results', null);
...
//				}
			...
			if(loadResults[i].billLineAmt != undefined) {
				...
				if (loadResults[i].tranType == 'Check') {
...
//	try {
		...
		for (var i = 0; i < s.length; i++) {
			...
			if(cleanRecons.contains(reconCheck) == false) {
...
//				
//				jeEntries.push(resultsArr[j].recID);
//			}

			
			if (resultsArr[j].tranType == 'Bill') {
					resultsArr[j].billLineAmt = (parseFloat(resultsArr[j].ogTotalPayment) / (parseFloat(resultsArr[j].exRate))).toFixed(2);
