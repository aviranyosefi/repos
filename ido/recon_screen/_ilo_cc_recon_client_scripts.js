...
function manual_recon_closure() {
	...
	deficitTotal = parseInt(nlapiGetFieldValue('custpage_summary_total'));
...
//	console.log(i);
			...
			for (var j = 1; j <= b_lines; j++) {
				...
				if(thisAcc == currAccount) {				
					billRec.setLineItemValue('item', 'custcol_ilo_recon_session_col', j, toUpdate[i].recon_sessionID);
					billRec.setLineItemValue('item', 'custcol_ilo_recon_check_col', j, 'T');
					billRec.setLineItemValue('item', 'custcol_ilo_recon_memo_col', j, toUpdate[i].recon_memo);
		...
		if(toUpdate[i].rec_type == 'Bill Credit') { // api points - 30
			...
			for (var j = 1; j <= cred_lines; j++) {
				...
				if(thisAcc == currAccount) {				
					vendCredRec.setLineItemValue('item', 'custcol_ilo_recon_session_col', j, toUpdate[i].recon_sessionID);
					vendCredRec.setLineItemValue('item', 'custcol_ilo_recon_check_col', j, 'T');
					vendCredRec.setLineItemValue('item', 'custcol_ilo_recon_memo_col', j, toUpdate[i].recon_memo);
