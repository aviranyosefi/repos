...
if(daysBefore == 7) {
	...
	var inArr = invForLTDSAS;
		...
		nlapiLogExecution("DEBUG", 'ToSend - line231', JSON.stringify(ToSend))
	...
	for (var i = 0; i < ToSend.length; i++) {
		...
		if (ToSend[i].invdays_overdue == 3) {
			...
			nlapiLogExecution("DEBUG", 'afterdueThree- line247', JSON.stringify(afterdueThree))
	...
	if(afterdueThree !=[]) {
		...
		afterdueThree.forEach(function(invoice) {
		   // console.log(element);
			...
			nlapiLogExecution('DEBUG', 'thisInvoiceContacts line663', JSON.stringify(thisInvoiceContacts))
			for(var i = 0; i<thisInvoiceContacts.length; i++) {
			...
			if(mailSubsidiary == "LTD") {
				...
				var checkthis = emailArr.concat(emailArr);
			...
			nlapiSendEmail(fromId,checkthis, sbj, msg, 'ar.il@jfrog.com', null, attachRec, attachment);
			
			
			nlapiLogExecution('DEBUG', 'checkthis - 3days', JSON.stringify(checkthis));
			nlapiLogExecution('DEBUG', 'emailArr - 3days', JSON.stringify(emailArr));
			}
