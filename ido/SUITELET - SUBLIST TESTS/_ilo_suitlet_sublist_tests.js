/**
* Implementation of a simple setup assistant with support for multiple setup steps and sequenti
al or ad-hoc step traversal.
* State is managed throughout the life of the user's session but is not persisted across sessio
ns. Doing so would
...
// assistant.addStep('companypreferences', 'Setup Company Preferences').setHelpText("Setup your <b>important</b> company preferences in the fields below.");
// assistant.addStep('enterlocations', 'Enter Locations').setHelpText("Add Locations to your account. You can create a location record for each of your company's locations. Then you can track employees and transactions by location..");
// assistant.addStep('enteremployees', 'Enter Company Employees').setHelpText("Enter your company employees.");
...
// assistant.addStep('summary', 'Summary Information').setHelpText("Summary of your Assistant Work.<br> You have made the following choices to configure your NetSuite account.");
// 
 ...
 if (request.getMethod() == 'GET')
 {
	 ...
	 assistant.setSplash("Welcome to the Small Business Setup Assistant!", "<b>What you'll be doing</b><br>The Small Business Setup Assistant will walk you through the process of configuring your NetSuite account for your initial use.." +
	 ",<b>When you finish</b><br>your account will be ready for you to use to run your business.");
	 }
	 ...
	 if (step.getName() == "companypreferences") {
		 ...
		 firstDayOfWeek.addSelectOption( selectOptions[i].getId(),selectOptions[i].getText() );
		 }
		 }
...
;
 ...
 ( cpStep.getFieldValue( 'customerwelcomemessage' ) );
	...
	er as
	 ...
	 /* 3. For all other actions (next, back, jump), process the step and redirect to assistant pa
	ge. */
	 else
	 {
	 ...
	 if (assistant.getLastStep().getName() == "enterlocations" && assistant.getLastAction() == "n
		 ext" )
		  {
		  // create locations
		 ...
		 function getLinkoutURL( redirect, type )
		 {
		  ...
		  url += 'customwhence='+ escape(nlapiResolveURL('suitelet',context.getScriptId(), context.getDe
		 ploymentId()))
		  return url;
		 } 
 }