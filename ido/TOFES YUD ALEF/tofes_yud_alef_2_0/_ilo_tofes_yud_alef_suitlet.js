function tofes_yud_alef(request, response){
	...
	//nlapiLogExecution('debug', 'openBalancePrimary_Dprn[key]', JSON.stringify(openBalancePrimary_Dprn['Computers']))
		...
		var checkLang = resultsForm.addField('custpage_check_lang', 'text', 'Language');
		checkLang.setDisplayType('hidden');
		checkLang.setDefaultValue(isHebrew)
...
//		
//		var stringify = JSON.stringify(AllTypes['Computers']);
...
var openBal = JSON.stringify(clean_openbalanceLines[0])
var checkKeys = getTypes();


var All_types = {};
var actualLines = [];

checkKeys.forEach(function(type) {
	All_types[type.type_name] = [];
});

var allAssetLines = [];
for(var x = 0; x<theLines.length; x++) {
	allAssetLines.push(theLines[x][0]);
}
for(var y = 0; y<clean_openbalanceLines.length; y++) {
	allAssetLines.push(clean_openbalanceLines[y]);
}

var typeKeys = Object.keys(All_types);

for(var i = 0; i<typeKeys.length; i++) {
	for(var j = 0; j<allAssetLines.length; j++){
		
		if(allAssetLines[j].asset_type == typeKeys[i]){
			

			All_types[typeKeys[i]][All_types[typeKeys[i]].length ] = allAssetLines[j];
...
for(var t = 0; t<typeKeys.length; t++) {
	...
	var items = All_types[typeKeys[t]]
	  ...
	  var TotalLineObj = {
			...
			items.push(TotalLineObj)
			actualLines.push(items)
}

var allLines = [].concat.apply([], actualLines);
...
allLines.push(TotalLineObj)

		for(var i = 0; i<allLines.length; i++) {

	
			
			if(allLines[i].asset != 'TOTALS') {
			resultsSubList.setLineItemValue('custpage_resultssublist_view',i+1 ,assetView(allLines[i].asset_internalid));
			}
			resultsSubList.setLineItemValue('custpage_resultssublist_type',i+1 ,allLines[i].asset_type);
			resultsSubList.setLineItemValue('custpage_resultssublist_asset',i+1 ,allLines[i].asset);
			resultsSubList.setLineItemValue('custpage_resultssublist_asset_desc',i+1 ,allLines[i].asset_description);
			resultsSubList.setLineItemValue('custpage_resultssublist_purchase_date',i+1 ,allLines[i].asset_purchasedate);
			resultsSubList.setLineItemValue('custpage_resultssublist_dprn_percent',i+1 ,allLines[i].asset_lifetime);
//			
			if(allLines[i].cost_acq != undefined) {
				resultsSubList.setLineItemValue('custpage_resultssublist_c_acquisition',i+1 ,allLines[i].cost_acq);
			}
			if(allLines[i].cost_disposal != undefined) {
				resultsSubList.setLineItemValue('custpage_resultssublist_c_disposal',i+1 ,allLines[i].cost_disposal);
			}
			if(allLines[i].cost_writedown != undefined) {
				resultsSubList.setLineItemValue('custpage_resultssublist_c_writedown',i+1 ,allLines[i].cost_writedown);
			}
			if(allLines[i].dprn_dprn != undefined) {
				resultsSubList.setLineItemValue('custpage_resultssublist_d_depreciation',i+1 ,allLines[i].dprn_dprn);
			}
			if(allLines[i].dprn_disposal != undefined) {
				resultsSubList.setLineItemValue('custpage_resultssublist_d_disposal',i+1 ,allLines[i].dprn_disposal);
			}
			if(allLines[i].dprn_writedown != undefined) {
				resultsSubList.setLineItemValue('custpage_resultssublist_d_writedown',i+1 ,allLines[i].dprn_writedown);
			}
			
			if(allLines[i].openbalance_cost != undefined) {
				resultsSubList.setLineItemValue('custpage_resultssublist_c_openbalance',i+1 ,allLines[i].openbalance_cost);	
			}else{
				resultsSubList.setLineItemValue('custpage_resultssublist_c_openbalance',i+1 ,'0.00');		
			}
			
			if(allLines[i].openbalance_dprn != undefined) {
				resultsSubList.setLineItemValue('custpage_resultssublist_d_openbalance',i+1 ,allLines[i].openbalance_dprn);	
		}else{
			resultsSubList.setLineItemValue('custpage_resultssublist_d_openbalance',i+1 ,'0.00');		
		}
			
			if(allLines[i].hasOwnProperty('openbalance') == true) {
				
				resultsSubList.setLineItemValue('custpage_resultssublist_c_openbalance',i+1 ,allLines[i].openbalance.openbalance_cost);	
				resultsSubList.setLineItemValue('custpage_resultssublist_d_openbalance',i+1 ,allLines[i].openbalance.openbalance_dprn);	
			}
			...
			if(allLines[i].hasOwnProperty('obRange') == true) {
				
				  checkAcq = parseFloat(allLines[i].obRange.ob_cost_acq);
				  checkDisposal = parseFloat(allLines[i].obRange.ob_cost_disposal);
				  checkWriteDown = parseFloat(allLines[i].obRange.ob_cost_writedown);
				  
				  checkDprn = parseFloat(allLines[i].obRange.ob_dprn_dprn);
				  checkDprn_Disposal = parseFloat(allLines[i].obRange.ob_dprn_disposal);
				  checkDprn_WriteDown = parseFloat(allLines[i].obRange.ob_dprn_writedown);
				...
				resultsSubList.setLineItemValue('custpage_resultssublist_c_openbalance',i+1 ,total_OBCOST.toFixed(2));
				resultsSubList.setLineItemValue('custpage_resultssublist_d_openbalance',i+1 ,total_OBDPRN.toFixed(2));
			}

			
		}//end of loop over allLines
	
	
var json1 = JSON.stringify(allLines)
...
function getAllActivities(accBook, subsidiary, search) {
	...
	if (allSelection != null) {
		...
		theResults.push({
			...
			tran_acc_period : line.getValue(cols[14]),
