...
function awt_control(request, response) {
    ...
    if (request.getMethod() == 'GET') {
      ...
      selectPeriodFrom.setDefaultValue('241')
 ...
 selectPeriodTo.setDefaultValue('255')
       ...
       // selectSubsidiary.setDefaultValue(subsid);
        ...
        nlapiLogExecution('debug', 'get allVendors', 'get allVendors')
        var allVendorsArr = [];
        ...
        try {
            ...
            nlapiLogExecution('debug', 'get getSelection', JSON.stringify(results))
...
function getSelection(dateFrom, dateTo, vendors, checkIfAllVendors, onlyActual) {
	...
	var searchAWT = nlapiLoadSearch(null, 'customsearch_ilo_awt_bill_pymnt_search');
...
function getSelectionAllVendors(dateFrom, dateTo, onlyActual) {
    ...
    var fromPeriod = nlapiLoadRecord('accountingperiod', dateFrom);
    var toPeriod = nlapiLoadRecord('accountingperiod', dateTo);
	//additional filters
	if(dateTo || dateFrom != '') {
	    nlapiLogExecution('debug', 'getSelectionAllVendors', fromPeriod.getFieldValue('startdate')+' - ' +toPeriod.getFieldValue('enddate'))
		searchAWT.addFilter(new nlobjSearchFilter('trandate', null, 'onorafter', fromPeriod.getFieldValue('startdate')));
		searchAWT.addFilter(new nlobjSearchFilter('trandate', null, 'onorbefore', toPeriod.getFieldValue('enddate')));
	}
