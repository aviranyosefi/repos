...
//		currencySelect.setDefaultValue('ILS')
		...
		if(bankSelected == 'KABBCE-EURC') { //RBS FOREX
			
			var RBSForex = buildTransactions_RBS_FOREX(fileContent);
			
			data = header+filedetails+RBSForex;
			
			downloadData(RBSForex, response, null, bankName, accountName)
		}
...
var data = header+filedetails+transactions+closingTags
	...
	downloadData(data, response, null, bankName, accountName)
//response.write(JSON.stringify(tranArr))
...
function buildTransactions_RBS_FOREX(fileContent) {
	...
	allLines.forEach(function(line) {
		...
		if(singleLineArr[3] != null || undefined || '') {
			...
			debit: singleLineArr[17],
			credit : singleLineArr[18],
//			trntype: singleLineArr[10],
//			fitid: singleLineArr[0],
			dtposted: singleLineArr[10],
//			name : singleLineArr[4],
//			balance: singleLineArr[6]
		});
	...
	for(var i = 1; i<tranArr.length; i++) {
		
//		var memo = ascii2utf(tranArr[i].name);
		
//		var line = "<STMTTRN> \n"
//			line += "<TRNTYPE>"+tranArr[i].trnamt+" \n"
//			line += "<DTPOSTED>"+formatDateYYYYMMDD(tranArr[i].dtposted)+" \n"
//			line += "<TRNAMT>"+removeLeadingZeros(tranArr[i].trnamt)+" \n"
//			line += "<MEMO>"+memo+" \n"  
//			line += "<NAME>"+tranArr[i].fitid+" \n" //tranlate the hebrew
...
//			line += "</STMTTRN> \n\n"
		...
		var line = "<STMTTRN> \n"
			line += "<DTPOSTED>"+tranArr[i].dtposted+" \n"
			line += "<TRNAMT>"+tranArr[i].debit+" \n"
			line += "<TRNAMT>"+tranArr[i].credit+" \n"
