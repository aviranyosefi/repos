...
function multiple_print_suitelet(request, response){
	...
	if (request.getMethod() == 'GET') {
	    ...
	    var selectSubsid = form.addField('custpage_ilo_multi_subsid','text', 'Subsidary', null, 'custpage_ilo_searchdetails');
		...
		var allInvoices = invSearch(startInv,endInv)
    ...
    var res_PrintLayout = resultsSubList.addField('custpage_resultssublist_printlayout', 'select', 'Print Layout');
    res_PrintLayout.addSelectOption('a', 'Detailed');
    res_PrintLayout.addSelectOption('b', 'Summarized');
    ...
    if(allInvoices != null) {
    	...
    	for(var i = 0 ; i<allInvoices.length; i++) {
    		...
    		var layoutType = '';
    		if(allInvoices[i].isDetailed == 'T') {
    			layoutType = 'a';
    		}
    		if(allInvoices[i].isSummarized == 'T') {
    			layoutType = 'b';
    		}
    		resultsSubList.setLineItemValue('custpage_resultssublist_printlayout', i +1, layoutType);
...
//			response.write(json); //not writing any values to screen
	...
	printChoiceForm.setScript('customscript_qlt_batchprintmail_client')
    response.writePage(printChoiceForm)
		...
		var params = {
				'custscript_qlt_jobstorun' : request.getParameter('custpage_ilo_invtoprint'),
				'custscript_qlt_printpreference' : printPreference,
				'custscript_qlt_emailsender' : getUserID,
				'custscript_qlt_emailreciver' : getUserMail,
				'custscript_qlt_printormail' : 'Print'
		}
		...
		nlapiScheduleScript('customscript_qlt_batchprintmail_ss', 'customdeploy_qlt_batchprintmail_ss_dep', params)
    ...
    emailChoiceForm.setScript('customscript_qlt_batchprintmail_client')
    response.writePage(emailChoiceForm)
		...
		var params = {
				'custscript_qlt_jobstorun' : jobsToRun,
				'custscript_qlt_printpreference' : printPreference,
				'custscript_qlt_emailsender' : getUserID,
				'custscript_qlt_emailreciver' : getUserMail,
				'custscript_qlt_printormail' : 'Mail'
		}
...
function invSearch(startInv,endInv) {
	...
	var searchFAM = nlapiLoadSearch(null, 'customsearch_qlt_isr_invoice_search');
	...
	if (allSelection != null) {
		...
		theResults.push({
			...
			customerName : line.getValue('altname', 'customerMain'),
			total : line.getValue('fxamountremaining'),
			taxtotal : line.getValue('taxtotal'),
			printEnglish : line.getValue('custentity_inv_il_english_layout', 'customerMain'),
			printHebrew : line.getValue('custentity_inv_il_hebrew_layout', 'customerMain'),
			isSummarized : line.getValue('custbody_ilinv_sum_layout'),
			email : line.getValue('custentity_2663_email_address_notif', 'customerMain'),
