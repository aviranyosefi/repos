...
function shaam_out(request, response){
    ...
    if (request.getMethod() == 'GET') {
       ...
       //search filters - include in Shaam Report filter is between 1 or 2 (1 == "YES", 2 == "NO")
        ...
        var results = nlapiSearchRecord('vendor', null, [new nlobjSearchFilter('subsidiary', null, 'is', Subsidiary), new nlobjSearchFilter('custentity_ilo_include_in_shaam_report', null, 'noneof', '2')], [new nlobjSearchColumn('custentityil_tax_payer_id'), new nlobjSearchColumn('vatregnumber'), new nlobjSearchColumn('companyname'), new nlobjSearchColumn('custentity_ilo_shaam_update')]);
        if (results != null) {
            for (var i = 0; i < results.length; i++) {
                ...
                row += vendor_number;
                row += spacing;
                if((vendor_payerid.indexOf("5") != -1) || (vendor_payerid.indexOf("9999") != -1)) {
                	if(vendor_payerid == vendor_regnumber){
                		vendor_payerid = '000000000';
                		if(vendor_regnumber.indexOf("9999") !=-1){
                			continue;
                		}
                	}
                }
