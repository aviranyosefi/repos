function gatherPaymentsOnLoad(type) {
    try {



        var sf_ref = nlapiGetFieldValue('custbody_sf_ref');
        nlapiLogExecution('debug', 'custbody_sf_ref', sf_ref)
        if (sf_ref != null || '' || undefined) {

            var otherPymts = getOtherPayments(sf_ref)
            ...
            if (otherPymts != []) {
                ...
                var strTotal = 0;
                var currency = '';
                for (var i = 0; i < otherPymts.length; i++) {

                    var line = i + 1;
                    var total = (Math.round(parseFloat(otherPymts[i].totalPymt) /10)*10).toFixed(2);
                    strHeb += '`' + otherPymts[i].currency + total + comma + otherPymts[i].trandate + comma + otherPymts[i].paymentMethod + comma + line + '`;';
                    strEng += '`' + line + comma + otherPymts[i].paymentMethod + comma + otherPymts[i].trandate + comma + otherPymts[i].currency + total + '`;';

                    strTotal += parseFloat(total);
                    currency = otherPymts[i].currency;
                }

                strTotal = round(strTotal, 1);
                var fields = new Array();
                var values = new Array();
                fields[0] = 'custbody_kab_other_pymts_eng';
                values[0] = strEng;
                fields[1] = 'custbody_kab_other_pymts_heb';
                values[1] = strHeb;
                fields[2] = 'custbody_kab_other_pymts_total';
                values[2] = currency + strTotal.toFixed(2);

                //update and submit invoice
                var updatefields = nlapiSubmitField(nlapiGetRecordType(), nlapiGetRecordId(), fields, values);
        ...
        nlapiLogExecution('debug', 'err on Load', err)
    }
}
