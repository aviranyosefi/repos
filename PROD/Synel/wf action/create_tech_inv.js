...
function createTech(data) {
    try {
        var rec = nlapiCreateRecord('customsale_vehicle_inventory_ded');
        //Header Fields
...
function getSparParts(caseID) {
    ...
    columns.push(new nlobjSearchColumn('custrecord_sp_item'));
    columns.push(new nlobjSearchColumn('custrecord_sp_quantity'));
    ...
    for (var i = 0; i < s.length; i++) {
        results.push({
            item: s[i].getValue('custrecord_sp_item'),
            qty: s[i].getValue('custrecord_sp_quantity'),
