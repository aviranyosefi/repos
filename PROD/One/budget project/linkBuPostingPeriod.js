...
function searchBU() {
    ...
    columns[6] = new nlobjSearchColumn('custrecord_bu_start_date');
    columns[6] = new nlobjSearchColumn('custrecord_bu_end_date');
    ...
    if (s != null) {
        for (var i = 0; i < s.length; i++) {
            Results[i] = {
                ...
                sDate: s[i].getValue('custrecord_bu_start_date'),
                eDate: s[i].getValue('custrecord_bu_end_date'),
                isempty: [0, 0, 0, 0, 0],
            }
...
function getBu(properties) {
    ...
    for (y = 0; y < BuResults.length; y++) {
        ...
        if (nlapiStringToDate(properties.date) >= nlapiStringToDate(curr.sDate) && nlapiStringToDate(properties.date) <= nlapiStringToDate(curr.eDate)) {
            var arr = [];
            switch (properties.code) {
                case '1':
                    arr = [1, 2, 3];
                    if (curr.account == properties.account && checkifempty(curr, arr) && (curr.subsidiary == properties.subsidiary || curr.subsidiary == '')) {
                        return curr.id;
                    }
                    break;
                case '2':
                ...
                case '3':
                    arr = [1, 2];
                    if (curr.account == properties.account && curr.pDepartment == properties.parentDepartment && checkifempty(curr, arr) && (curr.subsidiary == properties.subsidiary || curr.subsidiary == '')) {
                        return curr.id;
                    }
                    break;
                case '4':
                    arr = [0, 2, 3];
                    if (curr.pAccount == properties.parentAccount && checkifempty(curr, arr) && (curr.subsidiary == properties.subsidiary || curr.subsidiary == '')) {
                        return curr.id;
                    }
                    break;
                case '5':
                    arr = [0, 3];
                    if (curr.pAccount == properties.parentAccount && curr.department == properties.department && checkifempty(curr, arr) && (curr.subsidiary == properties.subsidiary || curr.subsidiary == '')) {
                        return curr.id;
                    }
                    break;
                case '6':
                    arr = [0, 2];
                    if (curr.pAccount == properties.parentAccount && curr.pDepartment == properties.parentDepartment && checkifempty(curr, arr) && (curr.subsidiary == properties.subsidiary || curr.subsidiary == '')) {
                        return curr.id;
                    }
                    break;
                case '7':
                    arr = [0, 1, 3];
                    if (curr.department == properties.department && checkifempty(curr, arr) && (curr.subsidiary == properties.subsidiary || curr.subsidiary == '')) {
                        return curr.id;
                    }
                    break;
                case '8':
                    arr = [0, 1, 2];
                    if (curr.pDepartment == properties.parentDepartment && checkifempty(curr, arr) && (curr.subsidiary == properties.subsidiary || curr.subsidiary == '')) {
                        return curr.id;
                    }
                    break;
                default:
