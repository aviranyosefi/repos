...
function awt_scheduled(type) {
    ...
    try {
        ...
        var awtRange = currentContext.getSetting('SCRIPT', 'custscript_ilo_awt_range');
        awtRange = awtRange.split('-');
        var awtVendors = currentContext.getSetting('SCRIPT', 'custscript_ilo_awt_vendors');
        var awtOnlyActual = currentContext.getSetting('SCRIPT', 'custscript_ilo_awt_onlyactual');
        var emailToUse = currentContext.getSetting('SCRIPT', 'custscript_ilo_awt_email');
        var senderToUse = currentContext.getSetting('SCRIPT', 'custscript_ilo_awt_sender');
        var subsidToUse = currentContext.getSetting('SCRIPT', 'custscript_ilo_awt_subsid');
        var template = currentContext.getSetting('SCRIPT', 'custscript_ilo_awt_template')

        //nlapiLogExecution('debug', 'awtRange', awtRange);
        //nlapiLogExecution('debug', 'awtVendors', awtVendors);
        ...
        var UniqueArrVendors = toUniqueArray(allVendors);
        //nlapiLogExecution('debug', 'UniqueArrVendors', JSON.stringify(UniqueArrVendors));

        var results, vendorArray;
        ...
        //all vendors
        if (selectedVendors.length === 0) {
            results = getSelection(fromDate, toDate, UniqueArrVendors, onlyActual);
            vendorArray = getOneVendor(UniqueArrVendors, results);
        ...
        //nlapiLogExecution('debug', 'separateReports', JSON.stringify(separateReports))
        ...
        //nlapiLogExecution('debug', 'vendor array', vendor);
...
function toUniqueArray(allVendors) {
    var UniqueArr = [];
    for (var i = 0; i < allVendors.length; i++) {
        if (UniqueArr.indexOf(allVendors[i]) === -1) {
            UniqueArr.push(allVendors[i]);
        }
    }
    return UniqueArr;
}
...
function getSelection(dateFrom, dateTo, vendors, onlyActual) {
    ...
    searchAWT.addFilter(new nlobjSearchFilter('voided', null, 'is', 'F'));
    ...
    do {
        ...
        for (rs in resultslice) {
            ...
            allSelection.push(resultSelection[resultslice[rs].id] = resultslice[rs]);
            searchid++;
        }
