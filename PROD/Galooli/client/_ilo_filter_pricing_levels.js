                    ...
                    if (a != null) {
                        if (pricingLevel == null) {
                            ...
                            for (var i = 0; i < getRefPrices.length; i++) {
                                if (getRefPrices[i].item == getItem) {
                                    nlapiSetCurrentLineItemValue('item', 'custcol_ilo_gal_ref_price', parseFloat(getRefPrices[i].price / nlapiGetFieldValue('exchangerate')).toFixed(2));

                                }
                                if (getRefPrices[i].item != getItem) {
                                    nlapiSetCurrentLineItemValue('item', 'custcol_ilo_gal_ref_price', parseFloat(getBasePrice / nlapiGetFieldValue('exchangerate')).toFixed(2));
                        ...
                        if (pricingLevel != null) {
                            ...
                            if (itemType != 'itemgroup') {
                                ...
                                nlapiSetCurrentLineItemValue('item', 'custcol_ilo_gal_ref_price', parseFloat(priceObj.price_1_ / nlapiGetFieldValue('exchangerate')).toFixed(2));
                            }
    ...
    if (type == 'item' && name == 'quantity' && role != '14' && currItem != '1967') {
        ...
        if (pricingLevel != null) {
            ...
            try {
                ...
                nlapiSetCurrentLineItemValue('item', 'custcol_ilo_gal_ref_price', parseFloat(customersBasePrice[0].unitPrice / nlapiGetFieldValue('exchangerate')).toFixed(2));

            } catch (err) {
                console.log(err)
                customersBasePrice = priceObj.price_1_;
                nlapiSetCurrentLineItemValue('item', 'custcol_ilo_gal_ref_price', parseFloat(customersBasePrice / nlapiGetFieldValue('exchangerate')).toFixed(2));
            }
        ...
        try {
            ...
            if (role != '14') {
                ...
                if (checkPricingType == '-1') {
                        ...
                        try {
                            ...
                            for (var i = 0; i < allItems.length; i++) {
                                if (lineItem == allItems[i].internalid) {
                                    ...
                                    nlapiSetCurrentLineItemValue('item', 'custcol_ilo_gal_ref_price', (itemBasePrice / nlapiGetFieldValue('exchangerate')).toFixed(2));
                                }
                            }
                    ...
                    //}
                    if (itemBasePrice != '') {
                        var toCalculate = (itemBasePrice / nlapiGetFieldValue('exchangerate')) * lineQty;
                        var getPercentage = GetPercentage(toCalculate, lineTotal);
                    }
                    else { var getPercentage = '0' }
                    debugger;
