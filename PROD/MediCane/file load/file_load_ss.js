...
function loadFile() {
    try {
        ...
        var getUserMail = context.getEmail();
        var getUserID = context.getUser();
        ...
        for (var i = 1; i <= fileLines.length; i++) {
            ...
            var slaItem = substringjs(lines[2]);// C
            var qty = substringjs(lines[3]); // D
            var customerNumber = substringjs(lines[5]); // F
            var total = substringjs(lines[6]); // G            
            var date = substringjs(lines[12]); // M
            var city = substringjs(lines[22]); // W
            nlapiLogExecution('DEBUG', 'slaItem :' + slaItem + ' qty: ' + qty, 'customerNumber: ' + customerNumber + ' total: ' + total + 'date: ' + date + ' city: ' + city);                        
            ...
            sla_Generate(slaItem, qty, customerNumber,total, date, city, period, i);
        ...
        attachRec['employee'] = getUserID //attach email to customer record   
        var subject = "SLA Sales report ";
        if (errorTable != '') {
            var body = "<h3><span style='color:blue; font-weight: bold ;'>The process ended successfully </span><br></h3>";
            body += '<p>File Name:' + fileName + '</p>'
            body += '<p>Error List :</p>'
            errorTable += '</table>';
            body += errorTable;
            try { nlapiSendEmail(getUserID, getUserMail, subject, body, null, null, attachRec, null, false); }
            catch (e) {
                nlapiLogExecution('DEBUG', 'nlapiSendEmail error', e);
            }
        }
        else {
            var body = "<h3><span style='color:blue; font-weight: bold ;'>The process ended successfully whitout error </span><br></h3>";
            body += '<p>File Name:' + fileName+'</p>'     
            try { nlapiSendEmail(getUserID, getUserMail, subject, body, null, null, attachRec, null, false); }
            catch (e) {
                nlapiLogExecution('DEBUG', 'nlapiSendEmail error', e);
