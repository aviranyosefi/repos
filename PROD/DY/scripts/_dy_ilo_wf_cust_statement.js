...
function workflowAction_sendstatement() {
    try {
        ...
        var context = nlapiGetContext();
        var templateId = context.getSetting('SCRIPT', 'custscript_template_id');
        if (templateId == null)
            templateId = -1;
        ...
        try {
            ...
            if (tosalesrep == 'T') {
                ...
                var custentity_sd_dunning = custrec.getFieldValue('custentity_sd_dunning');
                var custentity_ad_dunning = custrec.getFieldValue('custentity_ad_dunning');
                var custentity_cust_account_director = custrec.getFieldValue('custentity_cust_account_director');
                ...
                var custentity_emp_dun_notification = nlapiLookupField('employee', salesrep, 'custentity_emp_dun_notification');
                if (custentity_emp_dun_notification == '1' && custentity_sd_dunning == 'T') //emp 'YES' and customer SD DUNNING 'T'
                {
                    var salesrepemail = nlapiLookupField('employee', salesrep, 'email');
                    if (salesrepemail != null)
                        sendTo.push(salesrepemail);
                }
                var directore_notification = nlapiLookupField('employee', custentity_cust_account_director, 'custentity_emp_dun_notification');
                if (directore_notification == '1' && custentity_ad_dunning == 'T') //emp 'YES' and customer AD DUNNING 'T'
                {
                    var account_directore_email = nlapiLookupField('employee', custentity_cust_account_director, 'email');
                    if (account_directore_email != null)
                        sendTo.push(account_directore_email);
                }
        ...
        var entitynamefull = rec.getFieldText('entity');
        var entityname = entitynamefull.substring(entitynamefull.indexOf(' '));
        ...
        try {
            ...
            try {
                ...
                nlapiLogExecution('error', 'No Statement for ', entityname + entitynamefull);
            }
     ...
     emailMerger.setTransaction(rec.getId());
            var mergeResult = emailMerger.merge();
     

            //msg = msg.replace("transaction.dueDate", dueDate);
            var msg = mergeResult.getBody();
          msg = msg.replace('Dear ', 'Dear ' + entityname);
            //var msg = renderTemplate(body, rec);
...
function renderTemplate(body, rec) {
    try {
      //  var templateFile = nlapiLoadFile(templateFileId);
        var templateContent = "";//templateFile.getFieldValue('content');
        templateContent = '<?xml version="1.0"?><!DOCTYPE pdf PUBLIC "-//big.faceless.org//report" "report-1.1.dtd"><pdf><head></head><body>' + body;
...
template += '<pdf><head></head><body>' + body;

template += '</body></pdf>'; // end of template
     // template = template.replace(/div/g,"p");
           // nlapiLogExecution('debug', 'template', template)
      var renderer = nlapiCreateTemplateRenderer();
        renderer.setTemplate(template);
        renderer.addRecord('record', rec);
       var xml = renderer.renderToString();
      // var response = nlapiRequestURL("https://5246650.app.netsuite.com", null, null);
      nlapiLogExecution('debug', 'renderTemplate', xml)
        //var html = renderer.renderToResponse();
        return xml;
    } catch (err) {
