...
var employeeMail = 'sales@univopharma.com'//context.getEmail;
...
function creditCustomers() {
    ...
    try {
        ...
        if (customer_list.length > 0) {
            ...
            for (y = 0; y < cust_for_credit.length; y++) {
                ...
                try {
                    ...
                    creditMemo.setFieldValue('memo','זיכוי בגין עמידה ביעד'); 
                    creditMemo.setLineItemValue('item', 'item', 1, '1472'); //add credit item
                    ...
                    if (!isNullOrEmpty(submition)) {
                        sList.push({
                            ...
                            creditMemo: nlapiLookupField('creditmemo', submition, 'tranid'),
                            total: nlapiLookupField('creditmemo', submition, 'total'),
                            currency: nlapiLookupField('creditmemo', submition, 'currency',true),
                            invLines: '0',
                            status: 'succeed'
                        })
                        ...
                        for (i = 0; i < lineArr.length; i++) {
                            ...
                            for (j = 0; j < count; j++) {
                                if (lineArr[i].invoiceLine == invoice.getLineItemValue('item', 'line', j + 1)) {
                                    ...
                                    sList[sList.length - 1].invLines = lineArr.length;
                                    break;
                                }
                            }
                            ...
                            for (j = 0; j < count; j++) {
                                if (lineArr[i].soLine == so.getLineItemValue('item', 'line', j + 1)) {
                                    ...
                                    //sList[sList.length - 1].soLines = lines;
                                    break;
                                }
                            }
                        }
        ...
        summaryEmail(employeeMail);
    } catch (e) {
        nlapiSendEmail(employeeMail, employeeMail, 'An error occurred in Automatic Target Credit Memos creation process', 'Automatic Target Credit Memos creation process has terminated unexpectedly with the following error: ' + e);
        summaryEmail(employeeMail);
    }
}
...
function summaryEmail(employeeMail) {
    ...
    try {
        var date = new Date();
        var subject = "Monthly Credit of Target Meeting Customers";
        var body = '<html>' +
            '<head>' +
            '<style>' +
            'table, th, td {' +
        ...
        if (sList != null && sList != '') {
            ...
            // for th
            successTbl += "<tr><th>Customer </th><th>Credit Memo</th><th>Amount</th><th>Currency</th><th># of Related Invoices Lines</th><tr>";
            var i = 0;
            for (v in sList) {
                successTbl += "<tr><td>" + sList[v].customerName + "</td>";
                successTbl += "<td>" + sList[v].creditMemo + "</td>"
                successTbl += "<td>" + sList[v].total + "</td>"
                successTbl += "<td>" + sList[v].currency + "</td>"
