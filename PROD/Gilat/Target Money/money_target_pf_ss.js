    ...
    try {
        ...
        for (var j = 0; j < salesTarget.length; j++) {
            ...
            if (!isNullOrEmpty(sales) && !isNullOrEmpty(id)) {
                ...
                nlapiSubmitField('customrecord_target_new_money_recurrent', id, 'custrecord_nm_data_pf', JSON.stringify(amountData))
            }
        }  
    }
    catch (e) {
...
function SalesRepTargets() {
    try {
        ...
        columns[0] = new nlobjSearchColumn('custrecord_nmr_sales_rep');
        columns[1] = new nlobjSearchColumn('custrecord_nmr_year');

        var search = nlapiCreateSearch('customrecord_target_new_money_recurrent', null, columns);
        ...
        if (allSelection != null) {
            for (var i = 0; i < allSelection.length; i++) {        
                Results.push({
                    ...
                    sales_rep: allSelection[i].getValue("custrecord_nmr_sales_rep"),
                    year: allSelection[i].getText("custrecord_nmr_year") ,
                });
            }       
        }
    } catch (e) {
