...
function searchTran(sales, year) {
    try {
        ...
        filters.push(new nlobjSearchFilter('custrecord_sr_type', null, 'anyof', 2));
...
function createTran(sales, year, amountData) {
    ...
    rec.setFieldValue('custrecord_sr_type', 2);
...
function getTarget(period, sales, ProductFamily) {
    try {
        ...
        filters.push(new nlobjSearchFilter('custrecord_gt_type', null, 'anyof', 2));
...
function getEmpFromTran() {
    var revenueelementSearch = nlapiSearchRecord("salesorder", null,
        [
            ...
            ["mainline", "is", "F"],
            "AND",
            ["custbody_delta_recurrent_usd", "lessthan", "0.00"], 
            "AND",
            ["taxline", "is", "F"], 
            "AND",
            ["formulanumeric: {quantityshiprecv}", "notequalto", "0"], 
            "AND",
            ["formulanumeric: CASE WHEN {custbody_topic} = 'Permanent Disconnection' AND {custcol_site.custrecord_perm_after_temp_stop_billing} ='T' THEN 0 ELSE 1 END", "equalto", "1"],
        ],
        [
