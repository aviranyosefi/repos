...
function inv_batch_firstmail(request, response){
	...
	var prepaidInvoice = form.addField('custpage_prepaid_invoice', 'checkbox', 'Only Prepaid invoices', 'custpage_search_group');
	...
	response.writePage(form);
        ...
        var invoicePrepaids = request.getParameter('custpage_prepaid_invoice');
        ...
        var getResults = getSelection(getDateTo, getDateFrom, getSubsidiary, getCreatedBy, invoiceTickets, invoicePrepaids, financeRep);
...
function getSelection(dateTo, dateFrom, subsidiary, createdBy, invoiceTickets,InvoicePrepaid, financeRep) {
	...
	if(invoiceTickets == 'T') {
  	...
  	if(InvoicePrepaid == 'T') {
		searchid = 'customsearch_ilo_invoice_prepaid_search';
	}
	...
	var resultset = searchInv.runSearch();
    	nlapiLogExecution('DEBUG','filters:' , JSON.stringify(searchInv.getFilterExpression()));
