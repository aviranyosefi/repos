...
function inv_batch_zuora(request, response) {
    ...
    if (request.getMethod() == 'GET') {
        ...
        var res_Type = resultsSubList.addField(
            'custpage_resultssublist_type', 'text',
            'Transaction Type');
        //res_Type.setDisplayType('hidden');
        ...
        for (var i = 0; i < getResults.length; i++) {
            ...
            resultsSubList.setLineItemValue('custpage_resultssublist_type', i + 1, getResults[i].type);     
        ...
        nlapiLogExecution('DEBUG', 'afterset', '');
        response.writePage(resultsForm);

    }//end of first results page

    else if ((request.getParameter('custpage_ilo_end') != 'end') && ((request.getParameter('custpage_ilo_tosend') != '') || (request.getParameter('custpage_ilo_toremove') != ''))) {
        ...
        nlapiLogExecution('DEBUG', 'toSend', toSend)
        ...
        nlapiLogExecution('DEBUG', 'rev_ToSend.length', rev_ToSend.length);
...
function getSelection(dateFrom, dateTo) {
    ...
    filters[5] = new nlobjSearchFilter('custbody_ilo_invmail_yes_no_mailed', null, 'noneof', ['1'])
    filters[6] = new nlobjSearchFilter('custbody_ilo_marked_as_sent', null, 'is', 'F')
    filters[7] = new nlobjSearchFilter('email', 'customer', 'isnot', '')
    filters[8] = new nlobjSearchFilter('name', 'file', 'isnot', '')
