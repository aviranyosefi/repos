...
function virtualItemsSearch(itemOrCategory, type) {
    ...
    columns.push(new nlobjSearchColumn('custrecord_multi_parent_qty'))
    ...
    if (s != null && s.length > 0) {
        ...
        if (stItem != "" || ndItem != "" || rdItem != "" || fourthItem != "" || fifththItem != "" || sixth6thItem != ""
            ...
            virtualItemsPerItem.push({
                ...
                parentQty: checkParentQty(s[0].getValue('custrecord_multi_parent_qty')),
            });
        }
...
function getQty(virtualItemsPerItem, type, line) {
    try {
        ...
        if (isNullOrEmpty(virtualItemsPerItem[0][type + '_qty'])) { qty = 1; }
            ...
            qty = Number(virtualItemsPerItem[0][type + '_qty']) * Number(virtualItemsPerItem[0].parentQty) * Number(rec.getLineItemValue('item', 'quantity', line))
        }
        else {
            qty = Number(virtualItemsPerItem[0][type + '_qty']) * Number(virtualItemsPerItem[0].parentQty)* (Number(rec.getLineItemValue('item', 'custcol_cbr_period', line) / 12))
        }
...
function checkParentQty(parentQty) {
    if (isNullOrEmpty(parentQty)) parentQty = 1;
    return parentQty
}
