...
function createInvoices() {
    ...
    nlapiLogExecution('debug', 'invoiceRecs after validateSheet ', JSON.stringify(recs))
        ...
        try {
            ...
            for (var x = 0; x < currObj.length; x++) {
                ...
                if (invoiceID != null) {
                    ...
                    try {
                        updatePSInvoice_ISR(invoiceID, dateToUse)
                    } catch (err) {
...
function validateSheet(invoiceRecs) {
    ...
    for (var i = 0; i < sheetsId.length; i++) {
        var ps_invoice_number = nlapiLookupField('customtransaction_ps_timesheet_report', sheetsId[i], 'custbody_ps_invoice_number')
        if (ps_invoice_number == '' || ps_invoice_number == null || ps_invoice_number == undefined) {
...
function updatePSInvoice_ISR(invoiceID, dateToUse) {
    ...
    try {
        ...
        if (subsid == '22') {
            ...
            if (checkIfAdjust == 'T') {
                ...
                if (InvoiceLine != null || InvoiceLine != undefined) {
                    ...
                    for (var x = 0; x < SOLineCount; x++) {
                        ...
                        if (isOriginalLine == '1' && (psConversionLineID == checkPSLineID)) { //Original Line from SO
                            ...
                            SOLineValues.ogSalesPrice = ogSORec.getLineItemValue('item', 'custcol_cbr_so_original_unit_price', x + 1)
                ...
                if (InvoiceLine != undefined || SOLineValues.salesPrice != '') {

                    rec.setLineItemValue('item', 'custcol_cbr_so_original_unit_price', InvoiceLine, SOLineValues.ogSalesPrice)
                    ...
                    rec.setLineItemValue('item', 'rate', InvoiceLine, parseFloat(calcExRate * SOLineValues.ogSalesPrice).toFixed(2))
                } else {

                    var newOGInvoiceLineCount = rec.getLineItemCount('item');
                    var calcExRate = parseFloat(1 / newExRate)

                    for (var i = 0; i < newOGInvoiceLineCount; i++) {
                        try {


                            var salesPrice = rec.getLineItemValue('item', 'custcol_cbr_so_original_unit_price', i + 1)
                            rec.setLineItemValue('item', 'rate', i + 1, parseFloat(calcExRate * salesPrice).toFixed(2))

                        } catch (err) {
                            nlapiLogExecution('debug', 'err - updating sales price', err);
                            continue;
                        }
                    }
