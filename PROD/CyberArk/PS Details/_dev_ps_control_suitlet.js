function ps_control(request, response){
	 ...
	 for(var i = 0; i<selectValues.length; i++) {
		 selectCustomer.addSelectOption(selectValues[i].customer_id,selectValues[i].customer_text);
	 }
	 
	 var selectSO = form.addField('custpage_select_so','select', 'Sales Order', null,'custpage_batch_group');
	 selectSO.addSelectOption('','');
	 for(var i = 0; i<selectValues.length; i++) {
		 selectSO.addSelectOption(selectValues[i].SO_id,selectValues[i].SO_text);
	 }
...
function getSelectValues() {
    ...
    var savedSearch = nlapiCreateSearch('customtransaction_ps_timesheet_report', null, columns)
    var resultset = savedSearch.runSearch();
    var returnSearchResults = [];
    var searchid = 0;
	var results = [];
