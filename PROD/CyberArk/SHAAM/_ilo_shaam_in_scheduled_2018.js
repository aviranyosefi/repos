...
function shaam_in_scheduled(type) {
    ...
    var vendors_no_certificate = vendSearchNoCert(subsidToUse);
    ...
    function vendSearch() {
        ...
        filters[2] = new nlobjSearchFilter('isinactive', null, 'is', 'F'),
            filters[3] = new nlobjSearchFilter('vatregnumber', null, 'isnot', '999999999')
    ...
    nlapiLogExecution('debug', 'stage1', '')
    ...
    for (var line = 1; line < arrLines.length; line++) {
        ...
        shaamUpdates.push({
            vendorID: arrLines[line].substr(0, 15),
    ...
    nlapiLogExecution('debug', 'stage2', '')
    ...
    for (var i = 0; i < shaamUpdates.length; i++) {
        ...
        if (currentContext.getRemainingUsage() < 150 || timeexe >= 0.5) {
        ...
        nlapiLogExecution('audit', 'befpreupdating vendor:' + vendID, ' cert: ' + vendBKCert);
        if (vendBKCert == 0) {
            nlapiSubmitField('vendor', parseInt(vendID), 'custentityil_bookkeeping_certification', 1);
        }
        else {
            // if vendor  certificated approved
            if (vendors_no_certificate.indexOf(vendID) > -1) {
                nlapiSubmitField('vendor', parseInt(vendID), 'custentityil_bookkeeping_certification', 2);
            }
        }
        ...
        nlapiLogExecution("DEBUG", "stage 2.5", '');
        if (Object.keys(myHash).length == '0') {
            ...
            try {
                ...
                try {
                    nlapiLogExecution("DEBUG", "stage 2.6", '');
                ...
                values[0] = vendBKCert == '0' ? '1' : '2';
                fields[1] = 'custentityil_expiry_date';
                values[1] = vendBKExpdate;
                fields[2] = 'custentity_ilo_shaam_update';
                values[2] = 'T';
                nlapiLogExecution('debug', 'stage3', '')
                //update and submit vendor-level form
                nlapiLogExecution('debug', 'updating vendor:', JSON.stringify(fields) + ' ' + JSON.stringify(values));
                var updatefields = nlapiSubmitField('vendor', parseInt(vendID),
                    fields, values, 'T');

            } catch (err) {
                nlapiLogExecution('error', 'error creating first WHT Cert', err)
                continue;
            }
            ...
            try {
                ...
                nlapiLogExecution('error', 'err updating', err)
            }
...
function vendorView(checkid) {
    ...
    try {
        ...
        var empty = '';
        nlapiLogExecution('error', 'transactionView err', err);
        return empty;
    }
...
function getVendorName(checkid) {
    ...
    try {
        ...
        var empty = '';
        nlapiLogExecution('error', 'getVendorName err', err);
        return empty;
    }
...
function vendSearchNoCert(Subsidiary) {

    var filters = new Array();
    filters[0] = new nlobjSearchFilter('subsidiary', null, 'is', Subsidiary);
    filters[1] = new nlobjSearchFilter('custentity_ilo_include_in_shaam_report', null, 'noneof', '2');
    filters[2] = new nlobjSearchFilter('custentityil_bookkeeping_certification', null, 'anyof', '1');
    ...
    do {
        ...
        for (rs in resultslice) {
            vendors[resultslice[rs].id] = resultslice[rs];
            searchid++;
        }
