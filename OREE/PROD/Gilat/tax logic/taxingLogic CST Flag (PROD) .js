// 30.4.20 - changed tax codes for Belgum country (out of ec code would'nt work)
// 3.5.20 - tax group VAT (12.5%) GH on Sales (CST 0%) wasn't linked to Ghana subsidiary
            ...
            if (subsidiary == '24') {
                if (country == 'GH') {
                    ...
                    if (recordtype == 'noninventoryitem' && CSTFlag != 'T') {
                        ...
                        if (VI != 'T') {
                            //taxcode for ghana subsi &  ghana country & non-inventory = VAT (12.5%) GH on Sales // Old '1306'
                            rec.setLineItemValue('item', 'taxcode', y, '5519' );
                            nlapiLogExecution('debug', 'ghana country & non-inventory :', '5519');
                            changeFlag = true;

                        }
                        else {
                            //taxcode for ghana subsi &  ghana country & (non-inventory & VI) = VAT (12.5%) GH on Sales (CST 0%)
                            rec.setLineItemValue('item', 'taxcode', y, '4142');
                            nlapiLogExecution('debug', 'ghana country & (non-inventory & VI) :', '4142');
                            changeFlag = true;
                        ...
                        rec.setLineItemValue('item', 'taxcode', y, '4142');
                        nlapiLogExecution('debug', 'ghana country & inventory :', '4142');
                        changeFlag = true;
                    }
                }
                else {
                    //taxcode for ghana subsi & no ghana country = Zero rate GH
                    rec.setLineItemValue('item', 'taxcode', y, '84');
                    nlapiLogExecution('debug', 'no ghana country TaxCode 0% :', '84');
                    changeFlag = true;
                }
            }
            else {
