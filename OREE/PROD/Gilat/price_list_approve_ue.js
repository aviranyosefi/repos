function after_submit(type) {
    ...
    if (type != 'create') {
        ...
        var old_expDate = oldRec.getFieldValue('custbody_expiration_date');
        var new_expDate = newRec.getFieldValue('custbody_expiration_date');

        if (old_expDate != new_expDate) {
            
            var rec = nlapiLoadRecord('customtransaction_price_list', nlapiGetRecordId())
            rec.SetFieldValue('transtatus', 'A');
            var itemCount = rec.getLineItemCount('line');
            if (itemCount > 0) {
                for (var i = 1; i <= itemCount; i++) {
                    rec.setLineItemValue('line', 'custcol_pl_line_approval_status', i, 'F')
