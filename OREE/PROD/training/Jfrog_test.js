...
/*    var empId = nlapiGetContext().getUser();
    nlapiLogExecution('debug', 'email: ', context.email);
    if (empId != '244490') {
    ...
    try { 
        ...
        for (var i = 0; i < 10; i++) {
            ...
            try {
                if ((i % 3) == 0) {
                    nlapiDeleteRecord(s[i].getRecordType(), 'dsfds');
                }
                else {
        ...
        nlapiLogExecution('debug', 'Load search failed', e);
    }
...
function email(success, fail, scriptTypeN) {
    try {
        ...
        body += list;
        body += '<br></html>';
        nlapiSendEmail(244490, 244490, subject, body);
