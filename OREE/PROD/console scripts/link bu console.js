...
function getBu(properties) {
    ...
    for (y = 0; y < BuResults.length; y++) {
        var curr = BuResults[y];
        var arr = [];
        if (properties.parentRegion != '-1') {
debugger;
            arr = [0, 1, 3];
            if (curr.department == properties.department && curr.Pregion == properties.parentRegion && checkifempty(curr, arr) && curr.subsidiary == properties.subsidiary) {
                return curr.id;
            }
            continue;
        }
...
function properties_addition1(prop, record, index, itemType) {
    debugger;
    var region = record.getLineItemValue(itemType, 'cseg1', index);
    if (region != null && region != undefined && region != '') {
        if (prop.department != null && prop.department != undefined && prop.department != '') {
            var depControlLevel = nlapiLookupField('department', prop.department, 'custrecord_sim_budget_controled_by');
            if (depControlLevel == 'T') {
