...
function fieldchangePost(type, name) {
    if ((name == 'subsidiary') && !isNullOrEmpty(nlapiGetFieldValue('custbody_adjustment_reason')) && !isNullOrEmpty(nlapiGetFieldValue('subsidiary'))) {
        ...
        var acc = search_account(reason, subsidiary,1);
        if (acc != -1) {
...
function search_account(reason,subsidiary,mode) {
...
function ValidateLines(type, name) {
    var reason = nlapiGetFieldValue('custbody_adjustment_reason');
    var subsidiary = nlapiGetFieldValue('subsidiary');
    if (!isNullOrEmpty(reason) && !isNullOrEmpty(subsidiary)) {
        var sign = search_account(reason, subsidiary, 2);
        if (sign != -1) {
        ...
        alert('please enter values for Adjusnment Reason and Subsidiary fields first');
        return false;
    }
}