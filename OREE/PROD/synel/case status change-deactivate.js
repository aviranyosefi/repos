...
function scheduleStatusChange() {
    ...
    var res = specialDaysSearch(todayStr);
    if (res != -1) {
        if (res.type == '3') {
            ...
            if (hour == hourforcheck) {
                ...
                if (casesRes != -1) {
                    for (i = 0; i < casesRes.length; i++) {
                        try {
                            if (nlapiLookupField('supportcase', casesRes[i].Id, 'status') != '13') {
                                nlapiSubmitField('supportcase', casesRes[i].Id, 'status', '13');
                                ...
                                nlapiLogExecution('DEBUG', ' case: ' + casesRes[i].caseNumber + ' is already on status - שעות אי פעילות ', '');
                            }
                        }
                        catch (e) {
                ...
                nlapiLogExecution('DEBUG', today + '-special day ', hour + ' - is not the ending time');
            }
        }
    }
    else {
...
function casesSearch() {
    var search = nlapiLoadSearch(null, 'customsearch_cases_to_off');
    var s = [];
    var Results = [];
